...
for model in models:
    print(f'> Processing CommonSenseQA with model {model}')
    if model == 'llama3':
    ...
    out_cognitive = []
    out_knowledge = []
    for i, row in tqdm(winogrande_df.iterrows(), desc=f'Processing Winogrande with model {model}', total=len(winogrande_df)):
    ...
    with open(os.path.join(current_dir, 'output_winogrande', f'{model}_cognitive.json'), 'w') as f:
        json.dump(out_cognitive, f)
    with open(os.path.join(current_dir, 'output_winogrande', f'{model}_knowledge.json'), 'w') as f:
