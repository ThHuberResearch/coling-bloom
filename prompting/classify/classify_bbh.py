...
for model in models:
    print(f'> Processing BBH with model {model}')
    if model == 'llama3':
    ...
    for task in tqdm(tasks, desc=f'Processing MATH with model {model}', total=len(tasks)):
        ...
        with open(os.path.join(current_dir, 'output_bbh', f'{model}_{task_name}_cognitive.json'), 'w') as f:
            json.dump(out_cognitive, f)
        with open(os.path.join(current_dir, 'output_bbh', f'{model}_{task_name}_knowledge.json'), 'w') as f:
