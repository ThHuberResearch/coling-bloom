...
for model in models:
    print(f'> Processing TriviaQA with model {model}')
    if model == 'llama3':
    ...
    with open(os.path.join(current_dir, 'output_triviaqa', f'{model}_cognitive.json'), 'w') as f:
        json.dump(out_cognitive, f)
    with open(os.path.join(current_dir, 'output_triviaqa', f'{model}_knowledge.json'), 'w') as f:
