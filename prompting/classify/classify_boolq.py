...
for model in models:
    ...
    out_cognitive = []
    out_knowledge = []
    for question, answer, passage in tqdm(zip(boolq_data['question'], boolq_data['answer'], boolq_data['passage']), desc=f'Processing BoolQ with model {model}', total=len(boolq_data)):
    ...
    with open(os.path.join(current_dir, 'output_boolq', f'{model}_cognitive.json'), 'w') as f:
        json.dump(out_cognitive, f)
    with open(os.path.join(current_dir, 'output_boolq', f'{model}_knowledge.json'), 'w') as f:
